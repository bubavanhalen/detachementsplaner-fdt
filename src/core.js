...
function toast(text){$$('.toast').forEach(e=>e.remove());const el=document.createElement('div');const inDialog=$('#dlg')?.open;el.className='toast'+(inDialog?' dialog-toast':'');el.textContent=text;if(inDialog)$('#dlgB').prepend(el);else document.body.appendChild(el);const live=$('#liveMessage');if(live)live.textContent=text;setTimeout(()=>el.remove(),3500);}
function modal(title,body,buttons){const dlg=$('#dlg');$$('.toast').forEach(e=>e.remove());$('#dlgH').textContent=title;$('#dlgB').innerHTML='<div id="dialogError" class="notice dialog-error" role="alert" tabindex="-1" hidden></div>'+body;$('#dlgF').innerHTML='';
  for(const b of buttons||[{t:'Schliessen'}]){const el=document.createElement('button');el.className='button'+(b.primary?' primary':'')+(b.danger?' danger':'');el.textContent=b.t;if(b.id)el.id=b.id;el.disabled=!!b.disabled;el.onclick=()=>{const error=$('#dialogError');error.hidden=true;error.textContent='';try{if(!b.fn||b.fn()!==false)dlg.close();}catch(e){error.textContent=e.message;error.hidden=false;error.focus();error.scrollIntoView({block:'nearest'});}};$('#dlgF').appendChild(el);}
